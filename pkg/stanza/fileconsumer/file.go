...
func (m *Manager) closePreviousFiles() {
	...
	readers := m.previousPollFiles.All()
...
func (m *Manager) Stop() error {
	...
	if m.persister != nil {
		...
		for _, knownFiles := range m.knownFiles {
			checkpoints = append(checkpoints, knownFiles.All()...)
		}
...
func (m *Manager) poll(ctx context.Context) {
	...
	if m.persister != nil {
		...
		for _, knownFiles := range m.knownFiles {
			allCheckpoints = append(allCheckpoints, knownFiles.All()...)
		}

		for _, r := range m.previousPollFiles.All() {
...
func (m *Manager) consume(ctx context.Context, paths []string) {
	...
	for _, r := range m.currentPollFiles.All() {
		wg.Add(1)
...
func (m *Manager) makeReaders(paths []string) {
	...
	for _, path := range paths {
		...
		if r := m.currentPollFiles.MatchExact(fp); r != nil {
...
func (m *Manager) newReader(file *os.File, fp *fingerprint.Fingerprint) (*reader.Reader, error) {
	...
	if oldReader := m.previousPollFiles.MatchPrefix(fp); oldReader != nil {
		return m.readerFactory.NewReaderFromMetadata(file, oldReader.Close())
	}

	// Iterate backwards to match newest first
	for i := 0; i < len(m.knownFiles); i++ {
		if oldMetadata := m.knownFiles[i].MatchPrefix(fp); oldMetadata != nil {
